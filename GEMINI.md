...
## Source of truth
- **`design.md`** — the approved plan. **Check the assignment header in `task.md` for which section governs your current revision** (Rev 4.0 Sell Planner is governed by **Section C**). Section A = Contracts (the WHAT: interfaces, invariants, closed error taxonomy). Section B = Blueprint (the WHERE: stack, components, trust boundaries). This is the Standard-tier combined doc (it replaces the Regulated `contracts.md` + `blueprint.md`).
- **`task.md`** — your exact checklist. Check off `[x]` as you go. Do not add/remove tasks.

## What you MUST do
- Read `design.md` fully before writing any code, then work **only** the checklist under your revision's heading in `task.md` — earlier revisions are already shipped; leave their code and tests alone except to keep them green.
